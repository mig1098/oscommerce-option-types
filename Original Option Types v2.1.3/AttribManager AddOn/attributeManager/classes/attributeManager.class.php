...
//BOF - Zappo - Option Types v2 - Added Option Types v2 Sort order, and Removed Sort order from original AttributeManager (No more need for this)
			$queryString = "select * from ".TABLE_PRODUCTS_OPTIONS." where language_id='".amDB::input($this->intLanguageId)."' order by products_options_order";
...
// BOF - Zappo - Product Types - Products as Options - Set TEXT PRODUCT WARNING for Products as Options "Placeholder" Option Values
        if (defined('TEXT_PRODUCT_WARNING')) {
          $optionType_query = tep_db_query("select products_options_type from " . TABLE_PRODUCTS_OPTIONS . " where products_options_id = '" . $optionId . "' and language_id = '" . amDB::input($this->intLanguageId) . "'");
          $optionType = tep_db_fetch_array($optionType_query);
          $OptType = $optionType['products_options_type'];
        }
        if(!array_key_exists($optionId, $this->arrAllOptionsAndValues)) {
					$this->arrAllOptionsAndValues[$optionId]['name'] = $allOptions[$optionId];
          //BOF - Zappo - Option Types v2 - ONE LINE - Auto-Add values for uploads, text, etc.
          $this->arrAllOptionsAndValues[$optionId]['values'][0] = TEXT_UPLOAD_NAME;
        } elseif (defined('TEXT_PRODUCT_WARNING') && $OptType == OPTIONS_TYPE_PRODUCT) {
					$this->arrAllOptionsAndValues[$optionId]['name'] = $allOptions[$optionId];
          $this->arrAllOptionsAndValues[$optionId]['values'][0] = TEXT_PRODUCT_WARNING;
        }
      }
// EOF - Zappo - Product Types - Products as Options - Set TEXT PRODUCT WARNING for Products as Options "Placeholder" Option Values
		}
...
//            print_r($allProdOptionsAndValues);
            ...
            $data = array('template_id' => $newTemplateId,
                                                        'options_id' => $optionId,
...
// - Zappo - Option Types v2 - Removed QTPro, and Removed Sort order from original AttributeManager (No more need for this) 
                              $this->addAttributeToProduct(array('option_id' => $optionsId,
